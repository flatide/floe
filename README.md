...
- **텍스트 레이어 예산 — 무인 자동 처리** (v0.5.3): 양산 파일은
  도형마다 붙는 마커 텍스트가 수십억 개일 수 있다 (호스트 실측 17억
  개/754GB RSS로 수집 불능). 카운트 전용 패스가 레이어별 예산(기본
  100만, `FLOE_TEXT_CAP`, 0=무제한)을 판정하고 — 예산 초과 레이어도
  **드롭하지 않는다**: 타일마다 bbox 제한 질의로 최대 N개(기본 1만,
  `FLOE_TEXT_TILE_CAP`)씩만 수집해 **공간적으로 균일하게 솎는다**.
  근접 줌 pick은 die 어디서나 동작하고, 작업량은 텍스트 수와 무관하게
  타일수×N으로 유계라 17억짜리 레이어도 몇 분에 끝난다. 진짜 라벨
  레이어인지 사람이 판단할 필요가 없다 — 가벼운 레이어는 전량, 무거운
  레이어는 균일 표본. 솎임은 로그 + meta(`texts_thinned`) +
  `floe info`에 표시된다. clip이 실어 나르는 원본 텍스트는 밴드 경로
  에서도 제거되어(viewer-safe strip) 타일 경계 중복 없이 큐레이트된
  세트만 b0에 실린다.
- **캡 조정은 재인덱싱 없이**: `FLOE_TEXT_CAP=.. FLOE_TEXT_TILE_CAP=..
  floe index <src> --texts-only` — 소스 1회 read + **b0 타일의
  텍스트만 재작성**(b1~b3 무변경, 무관한 b0는 건너뜀) + skeleton
  재생성. 수십 분짜리 조정으로 끝난다 (전체 재타일링 대비).
