...
- **병합 트윈 (merged twin)** (v0.6.0): 컷으로 빠지는 밴드는 화면에서
  ...
  콘텐츠). **기본은 인덱스에서 제외**(v0.6.19, 사용자 결정: 인덱싱
  완주가 우선 — 트윈 없이도 뷰어는 동작하고 컷된 밴드가 사라질
  뿐이다). 트윈은 언제든 `floe index <src> --merge-only`로 재타일링
  없이 후添 가능(밴드 파일만 다시 읽음, 소스 읽기 없음; stress30
  실측 7초). 인덱스에 포함하려면 `--merge`.
