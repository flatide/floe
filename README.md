...
- **cut 미만의 대표(page frontier, 2026-09-17)**: 모든 도형이 cut보다 작은
  ...
  **대표만 남는다** — 컷 문턱에서 L레벨(옥타브당 2) 아래면 2^L개 중 하나를 지오메트리
  그대로 그린다: 페이지는 모두 남기고(디코드 예산 256 MiB 초과 시 index로 솎음) 그 안의
  레코드·반복 멤버를 래스터에서 솎으며, 배열 배치는 멤버를 축 균형으로 솎아 펼치고
  자식 셀은 통째로 그린다. wash 블록은 쓰지 않는다. 한 옥타브
  축소마다 뷰의 컷 항목은 4배, 남기는 비율은 1/4이라 뷰당 비용이 컷 시점 수준
  근처에 머물고(run이 뷰에 다 들어오면 그 줌의 레벨로 고정), 넓은 뷰에 보이는 것은
  가까운 뷰에도 있었다(축소 중 새로 나타나는 것이 없는 frontier의 성질). 요약처럼
