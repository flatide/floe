...
- **수집은 무인·유계, 예산은 스켈레톤 캡에서 역산** (v0.6.2): 양산
  ...
  레이어당 몫 = 4×캡 ÷ 텍스트 레이어 수, 초과 레이어는 타일별 bbox
  질의로 몫÷타일 수 개씩 **공간 균일하게** 표본화(질의는 캡에서
  조기 중단되어 작업량이 텍스트 수와 무관). 예산 도입 전에는 타일당
  1만 고정이라 1,600타일 호스트 칩에서 레이어당 1,600만 개(~10GB,
  ~5분)를 모았다가 5만으로 버리는 낭비가 실측됐다. `--text-cap` /
  `--text-tile-cap`을 명시하면 그 값이 우선.
...
- **시작 depth** (v0.6.1): 일반 오픈은 **depth 1**로 시작한다 —
  KLayout(`initial-hier-depth=1`)을 포함해 업계 뷰어들의 기본이 그렇듯
  거대 칩의 첫 화면을 빠르게 띄우기 위함이다. `--goto`나 `--drc`로
  특정 지점을 검사하러 들어갈 때는 **full**로 시작하고, `view --depth
  N`이 두 경우 모두를 덮어쓴다. 런타임 변경은 기존과 동일(숫자/`d`).
...
- **병합 트윈 (merged twin)** (v0.6.0): 컷으로 빠지는 밴드는 화면에서
  ...
  콘텐츠). 전체 인덱스에 기본 포함(`--no-merge`로 제외), 기존 캐시는
  `floe index <src> --merge-only`로 재타일링 없이 추가(밴드 파일만
  다시 읽음, 소스 읽기 없음).
