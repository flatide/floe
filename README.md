...
- 레이어 패널은 이름을 클릭해 온/오프하는 텍스트 목록이다 (체크박스 없음;
  꺼진 레이어는 **취소선**으로 표시). layer 번호로 그룹핑되며, 같은 번호에
  datatype이 여럿이면 (`11/0 M1`, `11/1 M1.1`) 최소 datatype이 `+M1` 부모
  행이 되고 나머지가 바로 아래 나열된다 — `+M1` 클릭은 그룹 전체를 함께
  토글하고, 자식 클릭은 그 자식만 토글한다. 자식 없는 레이어는 `+` 대신
  공백 한 칸을 붙여 모든 이름이 좌측 정렬된다.
