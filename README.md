...
- **텍스트 레이어 예산** (v0.5.1): 텍스트 수집이 레이어 단위로 돌며
  ...
  남기면 순회 순서상 die 한쪽 구석만 남아 더 나쁨). 카운트 전용 패스가
  먼저 돌아 **드롭 대상 레이어 전부가 수집 시작 전에 보고**된다.
  드롭은 로그 + meta(`texts_dropped`) + `floe info`에 표시.
  `FLOE_TEXT_CAP`으로 예산 조절, `0`이면 무제한(이전 동작).
- **드롭 복구는 재인덱싱 없이**: `FLOE_TEXT_CAP=<크게|0> floe index
  <src> --texts-only` — 소스 1회 read + **b0 타일의 텍스트만 재작성**
  (b1~b3 무변경, 텍스트 없던 b0는 건너뜀) + skeleton 재생성.
  필요한 라벨 레이어가 드롭된 걸 나중에 알아도 수십 분짜리 복구로
  끝난다 (전체 재타일링 대비). 반대로 캡을 낮춰 다시 줄일 수도 있다.
