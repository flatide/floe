...
- **cut 미만의 대표(page frontier, 2026-09-17)**: 모든 도형이 cut보다 작은
  ...
  **대표만 남는다** — 뷰 안의 컷 항목을 세어 프레임당 항목 예산(100만) 안에 드는
  레벨 L을 잡고 2^L개 중 하나를 지오메트리 그대로 그린다: 페이지는 모두 남기고(디코드
  예산 256 MiB 초과 시 index로 솎음) 그 안의 레코드·반복 멤버를 래스터에서 솎으며,
  컷 아래 배치는 멤버를 축 균형으로 솎아 멤버마다 자식 bbox 한 점을 그린다(자식 페이지
  디코드 없음). 배열 footprint 하나를 wash 블록으로 쓰지는 않는다. 한 옥타브
  축소마다 뷰의 컷 항목은 4배, 남기는 비율은 1/4이라 뷰당 비용이 컷 시점 수준
  근처에 머물고(레이아웃 전체가 뷰에 들어오면 예산만큼으로 멈춤), 넓은 뷰에 보이는
  것은 가까운 뷰에도 있었다(축소 중 새로 나타나는 것이 없는 frontier의 성질). 요약처럼
...
- 운영 튜닝은 shell 환경변수를 사용하지 않고 `view` 옵션으로 명시한다:
  `--lod on|off`, `--frames on|off`, `--labels on|off`,
  `--refinement on|off`, `--frame-cache on|off`, `--stream-kb`,
  ...
  `--perf-baseline`은 backend-neutral end-to-end 비교를 위해 refinement, exact
  frame cache, LOD, frame, label을 모두 끄고 독립 인스턴스를 연다. detail/depth는
  workload 자체이므로 호출자가 같게 지정한다.
...
- 레이어 패널은 텍스트 목록이다 (체크박스 없음): 이름 **더블클릭**으로
  ...
  아래로 펼쳐지고 마커가 `-`로 바뀐다 (다시 클릭하면 접힘; 시작 시
  모든 그룹이 펼쳐진 상태다). 선택된 행을 다시 클릭하면 선택이
  해제된다.
  한 `(layer, datatype)`에 여러 OASIS `LAYERNAME` exact/range가
  적용되면 원본 순서대로 `aliases`에 모두 저장하고 마지막 non-empty
  ...
  레이어 행은 기존 글꼴의 약 60% 크기를 사용하며 layer/datatype 열은
  데이터와 무관하게 `999.999` 고정 폭 안에서 좌측 정렬하고 약 0.2문자
  여백을 둔다. 컬러 표시 열은 기존의 2.5배인 5문자 폭이며 실제 도형과
  같은 1px 체크보드 speckle을 보여주고 가장자리는 해당 레이어 색의 1px
  실선으로 둘러싼다. layer name 앞에도 0.2문자 여백을
  두며 행 간격은 폰트 행 높이의 약 10%를 추가한다. 스크롤바
  공간은 항상 예약해 스크롤 중에도 목록 표시 폭이 변하지 않는다.
  긴 레이어 이름은 `...`로 축약하지 않고 전체를 배치하며, 패널 폭을
  넘으면 레이어 목록 하단의 가로 스크롤바로 이동해 확인한다.
  가로·세로 스크롤바 thumb은 검은 패널에서 보이도록 흰색으로 표시한다.
