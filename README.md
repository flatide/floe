...
초기보다 키우면 더 확실히 재현). 런처가 기본으로
`CAIRO_DEBUG=xrender-version=-1`(cairo 코어 프로토콜 폴백)을 설정해
이를 우회한다 — 프레임 전송이 약간 느려지는 대신 모든 크기에서 정상
표시된다. XRender가 정상인 X 서버(Exceed TurboX 등)에서 원복하려면
`FLOE_XRENDER=1`로 실행. (시스템 GTK venv 실행에서는 발생하지 않았고
포터블 번들의 cairo 조합에서만 확인됨.)
