...
| `g`, **Ctrl+.** | goto 다이얼로그 |
| `e` | **에러 박스 선택 모드**: 클릭 2점으로 박스 → 현재 룰의 박스 내 에러 전부 선택(선택 = gold, 상한 5000; highlight 여부 무관, v2 pack 필요). 그리드에서 **Ctrl+클릭** = 개별 토글, **Shift+클릭** = 범위 추가 |
| `n` / `p` | 현재 룰 안에서 다음 / 이전 위반 순환 (그리드 페이지 자동 이동) |
