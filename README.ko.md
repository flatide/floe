...
| `w` | **현재 에러 waive 토글** — 단클릭/n·p 포커스, 없으면 점프(더블클릭) 에러. v2 pack 필요 |
...
- **왼쪽 pane**: 상단 = **DRC 브라우저 상시 내장**(열기는 DRC 메뉴 ·
  룰 검색(목록 상단)+룰 목록[이름 · 에러수/waived]|에러 번호 그리드 ·
  하단 에러 상세; db 로드 시 pane이 자동으로 넓어지고, 좁히면 이름은
  "…" 줄임·상세는 줄바꿈·필터 행은 다단 래핑으로 대응), 하단 =
  미니맵(depth별 구조 프런티어 표시 — 인덱싱 때 실제 플래너로
  구워짐). cell/object 브라우저는 추후 같은 자리에 추가 예정.
- **중앙**: 캔버스. 상태줄에
...
- **오른쪽 pane**: 레이어 목록(Calibre식 `l.d ■ NAME`, 숨김 = 취소선만,
  ...
  **7×7 색 팔레트**, **5×4 fill 패턴 팔레트** (fit/clip은 메뉴).
