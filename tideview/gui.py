...
def _render_service(src, req, res):
    ...
    try:
        ...
        return
    tmp = os.path.join(tempfile.gettempdir(), f"tv_gui_{os.getpid()}.png")
    try:
        while True:
            job = req.get()
            if job is None:
...
class Viewer:
    def __init__(self, root, cache, server_sock=None):
        ...
        tk.Label(side, text="tideview", fg=ACCENT, bg=PANEL_BG,
    ...
    def _apply_cache(self, cache):
        ...
        self._pick_px = None
        src = self.meta["src"]
        self.root.title(f"tideview - {os.path.basename(src['path'])}")
    ...
    def open_file(self, path):
        ...
        if not c.exists():
            return f"ERR no index for {path}; run: tideview index {path}"
        c.load()
        self._apply_cache(c)
        return None
    ...
    def _clip_dialog(self):
        ...
        out = filedialog.asksaveasfilename(
            ...
            initialfile=f"tv_clip_{um[0]}_{um[1]}_{um[2]}_{um[3]}um.oas",
...
def run_viewer(cache, server_sock=None):
    try:
        ...
        sys.stderr.write("tideview: cannot open display %s "
