...
(function(root){
    ...
    function bind(o){
        ...
        let enabled=false,opened=false,busy=false,scope=null,generation=0,listing=0,prior=null,hidden=[];
        ...
        function current(){const c=o.context();return c&&scope&&c.view_id===scope.view_id&&c.state_rev===scope.state_rev;}
        function changed(){button.hidden=!enabled;button.disabled=!enabled||!o.context();
            ...
            if(opened&&scope&&!current()){el('share-consent').checked=false;note('View changed. Close and reopen this dialog to approve its current scope.');}
        }
        function close(restore){generation++;opened=false;scope=null;busy=false;panel.hidden=true;clearLink();el('share-consent').checked=false;button.setAttribute('aria-expanded','false');
        ...
        function open(){if(!enabled||opened){return;}const c=o.context();if(!c){return;}opened=true;scope={view_id:c.view_id,state_rev:c.state_rev};generation++;prior=doc.activeElement;
            ...
            el('share-cancel').focus();changed();refresh();}
        ...
        el('share-create').onclick=async function(){if(!enabled||!opened||busy||!current()||!el('share-consent').checked){return;}
            ...
            try{const v=await o.http('POST','/api/v1/shares',request);if(token!==generation||!opened){return;}
                if(!v||!['share_id','invite'].every(function(k){return /^[0-9a-f]{64}$/.test(v[k]);})||v.mode!==mode){throw Error('Invalid invitation response');}
            ...
            finally{if(token===generation){busy=false;el('share-consent').checked=false;changed();refresh();}}
        };
