...
const id='a'.repeat(64),invite='b'.repeat(64),requests=[],nodes=new Map();let current={view_id:'c'.repeat(64),state_rev:'1'},fail=false,entries=[];
class Element{
...
async function http(method,path,body){requests.push({method,path,body});if(method==='GET'){return {shares:entries};}if(method==='DELETE'){entries=[];return null;}
    entries=[{share_id:id,mode:body.mode}];if(fail){throw Error('timeout');}return {share_id:id,invite,mode:body.mode,invite_seconds:120,session_seconds:1800};}
...
(async()=>{
    ...
    c.init(true);el('share-open').onclick();await tick();assert.equal(requests.length,1);assert.equal(requests[0].method,'GET');assert.equal(el('share-create').disabled,true);
    await el('share-create').onclick();assert.equal(requests.length,1);el('share-consent').checked=true;el('share-consent').onchange();
    current={...current,state_rev:'2'};c.changed();await el('share-create').onclick();assert.equal(requests.length,1);assert(!el('share-consent').checked);
    ...
    console.log('WEB SHARE OWNER UI: ALL OK (default off, explicit approval, stale context, layout-only grant, private fragment link, revoke, uncertain outcome/no retry, lifecycle)');
