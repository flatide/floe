...
function environment(mode='explore',hash='#invite='+secret){
    ...
    class Element{
        ...
        getContext(){const self=this;return {fillRect(){self.pixels=new Uint8ClampedArray(self.width*self.height*4);},
    ...
    class XHR{
        ...
        send(body){assert.equal(location.hash,'','fragment must be removed before the first HTTP request');const entry={method:this.method,path:this.path,body:body?JSON.parse(body):null,headers:this.headers};requests.push(entry);
            ...
            else{assert.equal(this.headers['X-Floe-Guest-CSRF'],auth.csrf);this.status=sessionCode;v=this.method==='DELETE'?null:{share_id:id,mode,read_only:true,delivery:mode==='follow'?'follow_frames':'explore_frames'};}
    ...
    const c=Guest.bind({window:win,document:doc,location,history,XHR,WebSocket:WS,protocol:P,now:()=>clock,decode(h,data,cb){return Decode.create({ImageData:class{constructor(data){this.data=data;}},setTimeout:win.setTimeout,clearTimeout:win.clearTimeout},h,data,cb);}});
    ...
    function state(extra={}){return {type:'share.state',view_id:view,connection_epoch:epoch,dataset_revision:'1',worker_epoch:'1',state_rev:'1',render_rev:'1',render_key:'1',bbox_dbu:['0','0','64','32'],pixels:[64,32],depth:'full',detail:'high',thin:'keep',frames:true,labels:true,mono:false,...extra};}
