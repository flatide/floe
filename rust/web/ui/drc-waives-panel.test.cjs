...
(async()=>{
    ...
    assert.match(el('waives-status').textContent,/waiting for matching/);
    ...
    assert.match(el('waives-status').textContent,/current review metadata matches/);
    assert.doesNotMatch(el('waives-status').textContent,/waiting/);
