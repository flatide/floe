...
are bounded annotations, never a renderer substitute. M4a-5 adds manual two-point
rulers through owner `view.measure`: Rust computes coordinates, dominant-axis/free
segments and distances, optionally referencing this socket's successful snap ticket.
Cursor-only measurements do not require an exact scene and never rerender. Selection
gap/CD-order parity, clip and actual browser acceptance remain pending; deterministic
client tests are not pixel QA.
