...
rev 32 수정 (바닥 아웃라인은 가시 콘텐츠 없을 때만 — 2026-08-07,
0.11.7; 2차 실사용 보고: depth 2~5에서 같은 자리에 Calibre에 없는
소형 사각형):

- rev 32의 바닥 아웃라인을 무조건 방출하면 **완전히 전개된 셀 위에
  지오메트리와 겹쳐 박스가 남는다** — Calibre는 전개된 셀에 박스를
  그리지 않고, k+height ≤ D가 되는 depth부터는 위치가 고정이라
  "depth 불변 유령 사각형"으로 보인다.
- **확정 규칙**: 바닥 붕괴(norm_r → REM_FULL) 셀의 아웃라인은
  **가시 디자인 콘텐츠가 전혀 없을 때만**(lmask_rec ∩ vis = ∅)
  방출하고, 그때는 엣지를 대체한다(아래에서 나올 것이 없으므로
  하강도 생략). 콘텐츠가 보이면 지오메트리가 말하게 두고 박스 없음
  = Calibre와 동일. 전 레이어 off의 frame-only 점검에서는 아웃라인
  이 유일한 표현이므로 유지 — 1차 보고(줌인 소멸)도 재발하지 않음.
  r==0 depth-경계 프레임은 종전대로 마스크 무관(Calibre 동일).
- 유닛 above_cut_bottom_cell_keeps_outline 재작성: vis=∅에서 컷
  전환 프레임 집합 불변 + 엣지 0, vis=on에서 박스 없음·페이지 출하,
  frames off 침묵. fold 유닛 기대값 원복(BIG 아웃라인 없음).

rev 32 철회 (텍스트 없는 박스는 그리지 않는다 — 2026-08-07, 0.11.8;
3차 실사용 보고/결정):

- Calibre는 전 레이어 off에서도 박스를 보여주지만 그 박스들은 전부
  이름 텍스트를 동반한 frontier 박스다. rev 32(+수정)의 바닥
  아웃라인은 블록명이 붙지 않는 박스를 만들었고, **"텍스트가
  안나오는 박스는 그리지 않는다"**로 확정 — 바닥 아웃라인 방출을
  완전히 제거하고 pre-rev32 분류로 복귀했다.
- 결과 계약: 프레임 = r==0 depth-경계(블록명 fit 시 라벨 동반) +
  sub-cut fold(생략 콘텐츠의 줌아웃 대리, 컷 미만이라 어차피
  텍스트 fit 불가). 완전히 전개된 셀은 지오메트리로만 표현 — 전
  레이어 off면 아무것도 안 보이는 것까지 Calibre와 동일하며, 1차
...
rev 35 수정 (LOD 킬스위치가 화면 스케일을 지우던 결함 — 2026-08-07,
0.11.12; 실사용 보고 "모든 박스가 흰색, ...만 회색"):

- vfsd가 lod=0(사용자 테스트 상태)에서 req.px_per_dbu를 0으로
  소거 → rev 35 톤 판정이 흰색 폴백, rep fuse도 1 DBU로 오동작
  (frontier 외양은 LOD 토글 불변이어야 한다는 주석 계약 위반).
  라벨은 별도 label_px를 타서 "..."류만 회색으로 갈려 증상이
  드러났다.
- **수정**: lod=0/프로브는 opts.lod_k=0(문서화된 킬스위치)으로
...
rev 46 (미니맵 = 라이브 프런티어 — 2026-08-10, 0.11.27):

- 9.8G 실측: 구운 meta 프런티어가 depth별 스캔 예산(1M)을 DFS
  순서로 소진해 지역 단위 누락(구석 편향 포함). 굽기 휴리스틱
  보수 대신 사용자 제안 채택 — **미니맵 = fit 뷰가 그리는 프레임
  집합의 축소판**을 구조적으로 보장.
- vfsd `mode=frontier`(세션 불변): 캔버스 fit 스케일의 px/cut/
  depth로 plan_hier(rev 41/43/45 사다리 그대로) → WS 트리를
  월드 좌표로 전개(frontier_boxes: xf 합성 + 프레임/인스턴스 rep
  멤버, 8M 워크 가드) → 64×64 그리드 셀당 4개 스트리밍 keep →
  최대-우선 라운드로빈 ≤6000박스 → frontier_GEN.tsv
  (x0 y0 x1 y1 band). 결정적(순회 순서 + strict-greater 교체).
- 뷰어: (depth, cut_px) 키 지연 요청/캐시, 도착 시 미니맵 베이스
  무효화. **주의: 미니맵 px로 플랜하지 않음** — 컷이 px 기준이라
  180px 스케일이면 전부 소멸; 반드시 캔버스 fit px로 플랜 후
  다운스케일. detail 변경 = 새 키로 자동 재요청. 데몬 실패 시
  meta 폴백 고정(재요청 루프 없음). 서브픽셀 필터 2px→0.7px.
- meta 프런티어 굽기는 폴백(레거시 .ice·구 캐시·데몬 부재)으로
  유지. 재인덱싱 불필요, 포맷 불변.
