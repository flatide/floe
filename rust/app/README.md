...
현재 **일반 레이아웃 `index`만 지원**한다. 이름에 web이 있지만 HTTP 서버나
브라우저 UI는 아직 없다. 기존 Python `floe2`/GTK와 병행 개발하는 별도 실행 파일이다.

```sh
cd rust
cargo build --offline --locked --release -p floe-app -p floe-index
...
- `.jb`, `--level`, `info/render/probe/view/clip/...`는 미이관 오류를 낸다.
