...
| UI-04 pick/snap/측정 | query/inspect/measure/rulers + Rust query/export, 숫자·scene 유효성·stale gate. 실제 Chrome §5.3~5.5: 꼭짓점/변 스냅, Shift/Command 다중 선택, 자동 bbox gap2µm, 축 정렬20µm/자유각20.2237µm, CD/수동/자동 혼합 Undo와 CD 전용 삭제 | 실제 나머지 형상/다중 후보·여러 CD/지연 입력 경합·클립보드, 최신 hover/CSS 수정 재검증 및 Firefox/원격 입력. 이전 실행 파일 관측을 최신 BFCache 수용으로 세지 않음 |
