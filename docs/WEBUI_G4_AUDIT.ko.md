...
갱신: 2026-09-16, M4g-20(유도된 legacy 읽기). 상위 [계획](WEBUI_PLAN.ko.md), 원래 범위
...
|---|---|---|
| CLI 10종·보조 PNG 명령 | `rust/app`, `rust/app-core`; `validate_app_cli.py`, `validate_app_render.py`, `validate_app_jobdeck*.py`, `validate_fe_embed.py` 등. view 옵션/형식 대조 M4 §62; M4g-15a 명시 ICE/인접 reviewer·M4g-20 유도 legacy 읽기 | reviewer 읽기의 ASCII/cache 선택, 무효/개발 옵션의 최종 제품 경계. GDS/gzip은 기존 native 한계이며 새 지원 아님 |
...
| DRC-02 저장·전송 | reviewer 고정 sidecar, snapshot/prepare/approve·CAS·receipt, notes/waives/transfer HTTP와 UI gate. **M4g-14 확정 시 자동 저장 opt-in**, M4g-15a 명시 ICE 읽기/쓰기 분리·M4g-20 유도 legacy 읽기 | reviewer ASCII/cache 선택; 실제 브라우저 저장/충돌/복구 수용, 대형 sidecar 연속 저장 비용 실측 |
...
않는다. writer 옵션·별도 waive 파일과 혼용은 오류다. env 태그 자동 선택,
ASCII의 fresh ICE 선택은 아직 미이관이며 현재 help/시작 안내에서 범위를 표시한다.
...
1. reviewer 읽기의 ASCII/cache 경로(유도된 legacy 읽기는 M4g-20에서 연결),
   ...
   그 구현과 GTK 진단/애니메이션 PNG 경계는 남으며 기존 명령을 폐기하지 않는다.
