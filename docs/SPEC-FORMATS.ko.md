...
### .ice v2 — 완전 변환(pack, `--pack [--jobs N]`, 레이아웃 버전 3)

자기완결 포맷(.db 불필요): 룰 테이블이 파일 앞에 나열되고 에러는
룰(체크)에 그룹으로만 귀속된다. 에러당 고정 로케이터 없음.

```
[헤더 40B]  version=3(레이아웃 개정 카운터), flags=1
...
- **위치 쿼리** `IcePack.query_rect(µm rect, cap, checks=)`:
  체크 bbox → qbox(레코드 단위) → 해당 블록만 디코드 + 정확 bbox
  확정. 실측(95MB/50만 에러, 전 체크 다이 전역 산포의 최악 합성):
  단일 룰(뷰어 하이라이트 경로) 0.1~3ms · 전 룰 45~410ms ·
  full-die cap800 5ms.
...
- 리더 디스패치: 헤더 version 필드(1=사이드카 IceDb, ≥2=IcePack).
  **레이아웃 개정 규율**: pack 섹션 배치가 바뀌면 version을 올린다.
  리더는 현재 값(3)만 수용하고 옛 pack은 재-pack 안내와 함께 거부 —
