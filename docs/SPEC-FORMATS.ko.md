...
**리더 방어**(실덱 2026-08-21): 0.11.40 이전에 구운 pack은 소스
size/mtime 신선도로는 인덱서 수정을 감지 못해 해당 섹션이 남는다
— IcePack 오픈이 **꼬리의 `__RVE_*__` 체크를 잘라낸다**(프리픽스
전용 truncation: checks/es/bs/ecnt/cbb/wcount/total 동시 축소,
ci 인덱스·저장 전역번호 불변) + 재-pack 안내 stderr. 중간 위치는
저장 번호가 밀리므로 유지 + 안내만(재-pack이 정답).
