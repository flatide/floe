...
- **도형 픽셀 규칙 — area-true (2026-09-22, 사용자 결정, 뷰어 기본)**: 아래 §4의 KLayout 규칙은
  ...
  살 수 있는 멤버만 목록으로 만들어 걷는다(`GridRanks::survivors`). 목록은 멤버 폭의 상한(첫 멤버의
  ...
  목록 중 규칙을 통과한 멤버가 전수 검사의 그린 멤버와 순서까지 같다. 목록이 만들어진 145건은 8.2배
  적게 걷는다. `a_frame_draws_the_same_with_the_survivor_list`: 타일 2종, worker 1·3, c 1·4에서 프레임이
  바이트 동일하고 검사 멤버는 1/3 미만이다.
