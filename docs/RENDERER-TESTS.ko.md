...
- **도형 픽셀 규칙 — area-true (2026-09-22, 사용자 결정, 뷰어 기본)**: 아래 §4의 KLayout 규칙은
  ...
  **안쪽 테두리**(다각형·경로는 행 스팬의 끝과 위·아래 행이 덮지 않는 부분 — 밴드 밖 한 행씩도
  훑어 타일과 무관) — w px 도형은 평균 w px, g px 간격은 평균 g px; (2) 1px 미만 변이 있는 도형:
  그 변은 중심을 담은 한 픽셀, 긴 변은 픽셀 중심 스팬으로 그리되 **면적이 그 픽셀들을 채우는 비율**
  (선 w, 점 w×h)로 남기고, 남길지는 **세계 박스의 해시 순위**로 정한다(줌·pan·타일·워커와 무관,
  축소하면 부분집합, 정확한 중복은 같은 판정). 표시용 점(wash, wash 점, 저장 대표점)은 컷 아래를
...
occupancy·jobdeck 게이트는 그 페이지 렌더를 KLayout 규칙으로 고정해 비교한다. 대신 **바이너리 커버리지**(레이어당 on/off, oversampling 1,
