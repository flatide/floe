...
- **도형 픽셀 규칙 — area-true (2026-09-22, 사용자 결정, 뷰어 기본)**: 아래 §4의 KLayout 규칙은
  ...
  area-true는 (1) 1px 이상 도형: **픽셀 중심이 안에 드는 픽셀**만 칠하고 외곽선은 그 픽셀 집합의
  **안쪽 테두리**(다각형·경로는 행 스팬의 끝과 위·아래 행이 덮지 않는 부분 — 밴드 밖 한 행씩,
  화면 위쪽 밖의 행도 훑어 타일·화면 가장자리와 무관); (2) 1px 미만 변이 있는 도형: 그 변은 중심을
  담은 한 픽셀, 긴 변은 픽셀 중심 스팬으로 그리되 **면적이 그 픽셀들을 채우는 비율**로 남기고
  (사각형: 선 w, 점 w×h; 다각형·경로: 자기 면적 ÷ max(w,1)·max(h,1) — bbox 면적이 아니다), 남길지는
  **세계 박스의 해시 순위**로 정한다(줌·pan·타일·워커와 무관, 축소하면 부분집합, 정확한 중복은 같은
  판정).
  **보장 범위(리뷰 2026-09-22로 좁힘):** 1px 이상 도형에서 없어진 것은 윤곽선 때문에 **무조건**
  커지던 +1 px이다. 각 변은 여전히 픽셀 중심에서 0/1로 반올림되므로 폭·간격·밀도는 **위치(소수
  픽셀 위상)에 대해 평균으로만** 보존된다 — 한 위상에서는 아니다. 1.5/1.5 px 막대는 위상에 따라
  켜진 열이 0.667/0.333/0.333/0.667(평균 0.500), 3.8/1.2 px는 0.801/0.801/0.605/0.801(평균 0.752,
  덮임 0.760), 5.2/2.8 px는 모든 위상 0.628(덮임 0.650). 한 위상에서도 밀도를 맞추려면 경계
  픽셀의 부분 덮임을 보정해야 한다(미결정). 표시용 점(wash, wash 점, 저장 대표점)은 컷 아래를
  대신 보이는 점이라 KLayout 규칙 그대로다. exact 프레임과 `floe-render-cli`는 KLayout 규칙이고,
