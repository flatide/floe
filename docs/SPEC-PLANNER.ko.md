...
- 배열: 단일 배치는 자기 bbox(컷 미만). footprint가 박스 이하인 배열은 박스 하나. 더 넓은
  ...
  기울어진 grid는 종전대로 버린다.
- **박스는 rect 하나**(0.12.171): 박스가 대표하는 레이어 중 paint 순서상 **가장 위 레이어**에만
  그린다(`box_layers`; paint 순서 = (layer, datatype) 오름차순, 인덱스의 레이어 번호는 파일 등장
  순서라 다르다). 아래 레이어의 박스는 같은 픽셀을 덮고 덮어쓰이기 때문이다 — paint는 모두
  불투명이고 기본 채움(speckle)은 모든 레이어가 같은 위상이다. 레이어별 stipple을 쓰면 위
  레이어의 꺼진 픽셀로 아래가 비쳐야 하지만, 몇 픽셀짜리 박스의 실선 외곽 안쪽 일이다. 레이어
  집합은 paint 순위 비트셋(`LayerSet`, 최대 512개 레이어)으로 다룬다.
