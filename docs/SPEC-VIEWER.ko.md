...
- **Tab = 오버레이 3-상태 순환**(2026-08-21, 구 2-상태 토글에서
  변경; 2026-08-28 룰러를 1단계로 편입): `overlay_mode` 0→1→2→0.
  **0 = 모두 표시**. **1 = 점프된 에러만 유지** — 페이지 마커·gold
  선택 스탬프·**룰러(선+화살표+스냅+거리 칩)**·**note 패널**을 모두
  숨기고 점프 마크(+디자인 선택)만 유지, `_drc_hits`가 비어 숨은
  마커는 pick/툴팁 불가. **2 = 전부 숨김**(2026-08-19 flateyes
  패리티 상태) — `_draw_overlays` 조기 종료(**줌 밴드만 유지**),
  `_drc_hits` 비움. 룰러 선/스냅은 `_draw_overlays`가, 룰러 거리
  칩·note 패널은 위젯이라 `_update_labels`/`_update_note_labels`가
  각각 `overlay_mode==0` 게이트로 숨김(칩이 페인트 게이트와 별도로
  살아남던 실사고 이력). 상태줄로 현 상태 표기. ISO_Left_Tab 포함.
