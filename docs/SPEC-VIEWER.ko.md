...
- **filter errors in view**(체크박스, packed .ice v2 전용 — 구
  ...
  따라오게 함, 포커스 셀은 뷰에 남아 있으면 유지). **캔버스
  마킹·흑백 연동·더블클릭 자동 off는 폐지**(마커 붕괴 규칙이 시인성
  전담). 뷰·db·선택 룰 키로 캐시, 룰 미선택 시 상태줄 안내. v1
  사이드카에서는 토글 거부+안내.
...
- **에러 박스 선택**(`e`, 2026-08-14): 크로스헤어 모드에서 두
  ...
  여부 무관). 선택은 목록을 **대체하지 않는다**(2026-08-14 수정):
  그리드는 기존 내용(전체 또는 hl in-view)을 유지하고 선택된
  번호만 **gold 배경**으로 마킹(현재 셀 파랑이 우선). 캔버스는
  선택 에러를 **gold 원래 도형**(2px, 세그먼트 예산 20k)으로
  표시. **화면상 ≤2×2px인 에러는 어느 경로든 3×3 마커 사각형으로
  붕괴**(점프 마크 포함; 선택+포커스는 5×5 — 2026-08-14). 해제 = Esc(선택 해제 단계는 마크 해제보다 먼저) /
  룰 전환 / 새 박스.
- **단클릭 포커스**(`_drc_focus`): 에러 번호 단클릭 = 줌 유지, 상세
  갱신 + **해당 에러를 화면 중앙으로 팬**(goto 창폭 None = 줌
  불변, 필터 여부 무관). **외곽선 변화 없음**(halo 제거). 선택된
  에러가 마커로 붕괴한 경우 포커스는 5×5. 점프·룰 전환·Esc가
  해제.
