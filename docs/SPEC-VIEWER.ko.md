...
- **점프 규약**(`_drc_jump`): 줌 = 에러 전체 extent가 양축 모두 뷰의
  ...
  0.1µm 창). **CD 자동 룰러**(`_drc_cd_ruler`) 1개를 점프마다 교체
  부착: 단일 엣지=길이, 엣지 쌍=최근접 갭(평행 대면이면 중점 앵커;
  교차/접촉은 없음), 축정렬 사각형=좁은 변 중앙 관통. 복잡한
  폴리곤/엣지셋은 룰러 생략(사용자 규정 2026-08-13). 수동 룰러는
  보존, k/Esc에는 일반 룰러처럼 반응.
