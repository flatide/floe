...
- **룰 에러 상시 표시**(2026-08-14→15 수정): 룰이 열려 있으면
  **현재 그리드 페이지의 에러들**(_drc_page_marks — 그리드 채움 시
  ...
  변경이 마커에 즉시 반영. `_drc_stamp_errs` 공용 페인터(≤2×2px
  마커 붕괴 5×5·포커스 9×9·세그먼트 예산 20k). 선택(gold)은 그
...
- **mono(그레이스케일)**: `b` 키 토글(독립 — 필터와 연동 없음),
  ...
  무효화.
- **에러 박스 선택**(`e`, 2026-08-14→15): 크로스헤어 모드에서 두
  클릭이 박스를 정의 → **현재 그려진 마커(_drc_page_marks =
  필터·페이지 적용분) 중 박스 내 에러만** 선택 — 선택은 항상
  보이는 에러 대상(query_rect/v2 요구 제거, v1도 동작; 빈 박스는
  선택 해제). 선택은 목록을 **대체하지 않는다**(2026-08-14 수정):
  그리드는 기존 내용(전체 또는 hl in-view)을 유지하고 선택된
  번호만 **gold 배경**으로 마킹(현재 셀 파랑이 우선). 캔버스는
  선택 에러를 **gold 원래 도형**(2px, 세그먼트 예산 20k)으로
  표시. **화면상 ≤2×2px인 에러는 어느 경로든 5×5 마커 사각형으로
  붕괴**(점프 마크 포함; 포커스는 9×9 — 2026-08-14). 해제 = Esc(선택 해제 단계는 마크 해제보다 먼저) /
  룰 전환 / 새 박스.
