...
- **에러 박스 선택**(`e`, 2026-08-14): 크로스헤어 모드에서 두
  ...
  선택 에러를 **gold**로: hl ON = gold 사각형이 cyan 사각형 위에
  덮임(3×3/포커스 5×5), hl OFF = 원래 도형(gold 2px, 세그먼트
  예산 20k). **화면상 ≤2×2px인 에러는 어느 경로든 3×3 마커
  사각형으로 붕괴**(점프 마크 포함 — 2026-08-14). 해제 = Esc(선택 해제 단계는 마크 해제보다 먼저) /
  룰 전환 / 새 박스.
- **단클릭 포커스**(`_drc_focus`): 에러 번호 단클릭 = 줌 유지, 상세
  갱신 — highlight 모드면 5×5 사각형(뷰 이동 없음), 아니면 **해당
  에러를 화면 중앙으로 팬**(goto 창폭 None = 줌 불변). **외곽선
  변화 없음**(2026-08-14: halo 제거). 점프·룰 전환·Esc가 해제.
