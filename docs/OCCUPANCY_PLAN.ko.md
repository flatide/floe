...
|---|---|---|
| 벽시계 | 39.7 분 (2,383 s) | 9.9 분 (594 s), 4.0배 |
...
줄 유무로 가린다. 덱 소스는 667개(사용자 확인, 153은 pass 수): 소스당 평균 8-a
3.6 s·14.7 MB, 8-c 0.9 s·0.26 MB이므로 대부분 소스의 레이어가 비어 있고 채워진
17 MB급 소스는 열 개 안팎이라는 (a)가 유력하다(status 집계로 확정). 스레드 48개가
레이어마다 level 0 비트맵(추출본 6.7 MB)을 하나씩
