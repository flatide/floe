...
1. DRC를 trusted launcher에서 등록하고 HTTP에는 opaque ID만 제공. 등록한 pack만
   읽는 전용 유계 actor를 두어 query가 render/input reactor를 막지 않게 한다.
   rule/error 목록·검색·waive 필터·goto·marker overlay를 브라우저에 연결한다.
