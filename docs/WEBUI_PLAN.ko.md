...
작성 2026-08-29, 갱신 2026-09-15(M4g-8b 파일 선택 시 창 표시 설정 유지).
...
미색인 파일의 동의→색인→자동 재열기는 다음 단계이며 아직 암묵 수행하지 않는다.
...
2026-09-15, M4g-8b 기준. **로컬 Rust/web 대체 기능은 후반부지만 전체 계획의
...
다음 로컬 구현은 미색인 파일의 **동의→색인→자동 재열기**다. 그 뒤 bare FILE,
