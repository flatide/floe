...
작성 2026-08-29, 갱신 2026-09-15(M4g-11e 다중 스타일 편집).
...
2026-09-15, M4g-11e 기준. **로컬 Rust/web 대체 기능은 후반부지만 전체 계획의
...
| M2 | 로컬 DRC 조회·조작·저장/전송 구현 | **공유 게스트 URL/읽기 전용 권한·원격 배포는 미구현**(`shares=false`, loopback-only) |
...
다음은 이름 있는 색/채움 프리셋과 행 단위 편집 UX의 GTK 대조, 미이관 view 옵션·
파일 형식 차이 등 M0 잔여다. 프리셋 팔레트·bitmap 직접 편집을 현재의 색 picker와
hex-row 입력만으로 대체 완료했다고 세지 않는다. upstream occupancy M5의 실측 완료는 여기의
웹 M5(world-tile) 완료나 웹 성능 수용을 뜻하지 않는다.
GTK의 bitmap 슬롯 편집은 `FLOE_FILL_EDIT` 개발 옵션 뒤에 있으므로 일반 프리셋 선택과
구분해 이관 범위를 확인한다([M4 §60](WEBUI_M4.ko.md)).
