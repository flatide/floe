...
2026-09-15, M4g-12 기준. **로컬 Rust/web 대체 기능은 후반부지만 전체 계획의
...
별도 이관/개발 도구 경계 판정이 남는다([M4 §61](WEBUI_M4.ko.md)).
실제 브라우저의 저장·복구·입력 수용, Python-free Linux 실행, G1/G4 전체 검증도
남아 있다. M2 공유와 M3 현장, M5 조건부 작업을 이 로컬 마감과 혼동하지 않는다.
다음 로컬 단계는 G4 기능 목록 전체를 최신 코드·gate에 대조해 아직 실행 경로가 없는
항목과 정책 결정 항목을 분리하는 것이다. 단위/API/DOM gate를 실제 브라우저 수용으로
