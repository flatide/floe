...
2026-09-16, M4g-16c 기준. **로컬 Rust/web 대체 기능은 후반부지만 전체 계획의
...
별도 이관/개발 도구 경계 판정이 남는다([M4 §61](WEBUI_M4.ko.md)). M4g-16a에서
...
wire 참조 할당을 연결한다([M4 §68](WEBUI_M4.ko.md)). 웹 프리셋의 참조 할당·슬롯 편집
UI와 실제 브라우저 수용은 남아 있으며 UI-03 완료로 세지 않는다.
...
([M4 §65](WEBUI_M4.ko.md)). ASCII의 최신 ICE 자동 선택·legacy 임시 파일 경로는
아직 미이관이다. 개발 도구/CLI 경계도 별도로 남는다.
