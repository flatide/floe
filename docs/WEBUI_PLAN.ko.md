...
2026-09-15, M4g-11c 기준. **로컬 Rust/web 대체 기능은 후반부지만 전체 계획의
...
M4g-11c는 접기·페이지 간 범위의 서버 조회와 무렌더 검증까지 완료하며, UI가 아직
이 API를 사용하지 않으므로 사용자 조작 완료로 세지 않는다.
다음은 Ctrl/Shift 선택·접기/펼치기·페이지 간 선택 UI, 남은 레이어 조작, 미이관 view 옵션·
파일 형식 차이 등 M0 대조표 잔여다. upstream occupancy M5의 실측 완료는 여기의
