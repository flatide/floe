...
작성 2026-08-29, 갱신 2026-09-15(M4g-10 파일명 생략 실행·goto 복원).
...
2026-09-15, M4g-11a 기준. **로컬 Rust/web 대체 기능은 후반부지만 전체 계획의
...
M4g-11a에서 추가했다. 다음 우선순위는 §11에 따른 `feature/jobdeck` 합류다.
현재 로컬 `09be2ab`까지 미합류 커밋 16개가 있으며 occupancy 기본 생성·레이어별
depth 판정·희소 sub-cut 표시 변경이 포함된다. 이번 커밋에는 아직 포함하지 않는다.
합류 후 Rust 서비스의 대응과 회귀를 검증하고 선택·접기/펼치기·페이지 간 선택 UI,
남은 레이어 조작, 미이관 view 옵션·파일 형식 차이 등 M0 대조표 잔여를 진행한다.
