...
M4g-56은 늦은 뷰 닫기 응답이 복원된 새 뷰를 지우는 경합과 `closed`/없는 뷰의
이전 화면 잔류를 재현·수정했다.21개 JS 조합·전체 UI·실제 native 닫기/재열기는
검증했으며 실제 브라우저 BFCache 수용은 별도다
([M4 §113](WEBUI_M4.ko.md#113-m4g-56--늦은-뷰-닫기-응답과-닫힌-화면-복원)).
M4g-57은 같은 페이지의 복원 조회 응답 역전을 요청 순번·연결 세대·상태 revision으로
차단한다.33개 결정적 조합에서 최신 화면/닫기 보존과 현재 오류·새 revision 수용을
대조했다. 실제 브라우저 종합 수용과 구분한다
([M4 §114](WEBUI_M4.ko.md#114-m4g-57--동일-페이지의-복원-조회-응답-역전)).
M4g-58은 native 시작 시간 초과의 실행 전 구간을 관측하고 개발 게이트의 빌드/실행
시간을 분리했다. 정규 전체 배터리는 통과했지만 간헐적 시작 지연의 원인 해결,
실제 브라우저·Linux·현장 수용이나 G1/G4 완료로 세지 않는다
([M4 §115](WEBUI_M4.ko.md#115-m4g-58--native-시작-지연의-단계별-관측)).
M4g-59는 승인된 새 합성 서버를 직접 실행하고 실제 Chrome에서 같은 규칙의2오류
메모/waive 저장·reload 재조회, 준비 전 note 외부 변경의 거부/초안 유지·명시 재조회
후 재승인을 확인했다. 결과 불명/게시 직전 충돌 및 전체 수용은 남긴다
...
- 2026-09-18: 에이전트가 새 합성 서버를 실행하고 새 일회용 loopback 링크로 Chrome을
  여는 것을 승인했다. 별도 임시 DRC 복사본의 다중 오류 메모/waive 저장·충돌·결과
  불명 복구 검사도 승인했다. 기존 서버/메모 보존, 실제 설계·외부 서버 제외이며
  승인 자체를 실행/수용 완료로 세지 않는다.
- 2026-09-18: Python-free Linux 검증을 위한 GitHub Actions workflow 추가/실행은
  **보류 — 현장 Linux 검증 대기**. 비용이 발생하는 CI나 외부 실행을 임의로 시작하지 않는다.
