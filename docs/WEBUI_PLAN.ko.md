...
작성 2026-08-29, 갱신 2026-09-15(M4g-9a 승인된 색인→재열기 서버 작업).
...
작업으로 실행한다([M4 §53](WEBUI_M4.ko.md)). **브라우저 승인 창·복구 연결은 아직
미완료**이며 파일 선택만으로 실행되지 않는다.
...
2026-09-15, M4g-9a 기준. **로컬 Rust/web 대체 기능은 후반부지만 전체 계획의
...
다음 로컬 구현은 미색인 파일의 **동의→색인→자동 재열기 UI**다. 서버 작업은 M4g-9a에서
추가했지만 승인 preview·동일 요청 복구·성공한 새 뷰로의 UI 연결은 남아 있다. 그 뒤 bare FILE,
미이관 view 옵션·파일 형식/조작 차이의 명시 처리 등 M0 대조표 잔여를 닫는다.
