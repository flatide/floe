...
RENDERER-TESTS §3), O = 1멤버 조각(`Rep::One`, 해시로 돌아가 역시
선택이 바뀐다). 직렬·P1·P2(prefix가 직접 방출한 페이지 포함)가 같은
수를 센다(`p2_prefix_counts_the_grid_pieces_it_emits`). 셀 정의에
저장된 조각 수이지 화면 영향의 크기가 아니다 — 조각 하나가 든 셀이
반복 배치되면 배치마다 보인다. 실칩에 이 경로가 있는지(R·O가 0인지)
를 이 줄로 본다.
