...
### 12차 (4건, 점유 요약 M1~M4 구현 리뷰 2026-09-11, RENDERD 0.12.82)

판정과 조치는 `docs/OCCUPANCY_PLAN.ko.md` §12 "2차 리뷰" 표. 요지: 배치 반복
오프셋 벡터 제거(멤버별 charge), 거부된 path → 레이어 `none:unsupported`, 오차
계약을 1 px 팽창 규칙으로 정정하고 픽셀 단위 gate 추가, `.ovo` 비트맵 오프셋
(테이블 뒤·순서·겹침) 검증.
...
- **현장 2026-09-10: 특정 줌부터 사라지는 반복 도형.** 35.8 × 34.6 mm 소스에서
  ...
  **실측 7(2026-09-11, 문제 영역 26 × 33 mm·레이어 하나, `occupancy_experiment`)**:
  ...
  같은 추출본이 jobs 48로 4.4 s·17 MB(8-a 41.1 s·34 MB), 덱 전체 39.7 분 → 9.9 분
  (CPU 총량 동일, 평균 9.4코어; 쓰기 9.8 GB → 0.17 GB는 원인 확인 중). 뷰어 실측
  (8-b)은 대기. 실칩을 쓸 수 없을 때는
