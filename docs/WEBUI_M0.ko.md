...
연결했다. M4g-20은 유도된 legacy 임시 파일만 읽도록 연결했다. ASCII/cache 선택과
실제 브라우저/현장 수용은 남는다([M4 §75](WEBUI_M4.ko.md)).
...
| `--floe-reviewer` | 명시 ICE의 인접/유도 legacy note·waive 읽기만 지원(M4g-15a/20). ASCII/cache 선택은 아직 오류. `--drc-reviewer`는 쓰기 opt-in이므로 자동 alias 금지 |
