...
- `shape_cut`(tools/validate_shape_cut.py, 약 5초; `planner`·`render` 별칭에 포함): klayout.db로 만든
  ...
  0.2 µm 선은 그려지며 선만 있는 페이지는 남는다. `FLOE_RUST_SHAPE_CUT=arrays`(0.12.212, §10.9): 같은 넓은 뷰에서
  0.4 µm 정사각형 배열(OASIS 반복)이 킬 스위치와 같은 픽셀을 켜고, 따로 둔 0.3 µm 정사각형(1.9 px)은 잘리며,
  배열 밖은 max와 같다. 같은 선 12개를 자식 셀(선 하나짜리 셀 12배치)로 둔 레이아웃(리뷰
