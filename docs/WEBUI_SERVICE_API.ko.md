...
아래 전체 URI가 그대로 구현된 것은 아니다. M2b-1의
[로컬 초대·인증 API](WEBUI_SHARING_GRANTS.ko.md)와 M2b-2의
...
조회/측정 명령을 추가한다. M2b-3b2의 [명시 DRC 공유 API](WEBUI_SHARING_DRC.ko.md)는
guest별 `/drc`, `/drc/read`, `/drc/panel`, `/drc/selection`을 연결한다.
M2b-4a의 [로컬 초대·guest 화면](WEBUI_SHARING_UI.ko.md)은 `--local-sharing` 아래
정적 `/guest/{id}` shell과 별도 guest client를 연결한다. source/인증은 shell에 넣지 않는다.
guest DRC/layer/query/룰러 UI·원격 API 전체가 구현된 것은 아니다.
...
M2b-4b3의 [guest query/룰러·pointer UI](WEBUI_SHARING_QUERY_UI.ko.md)는 기존 네 개의
`explore.*` 조회/측정 명령만 사용한다. endpoint·native geometry 정책·권한은 추가하지 않는다.
M2b-4b4의 [DRC 순회·박스 선택](WEBUI_SHARING_DRC_NAV.ko.md)은 기존 guest read의
...
불변 PNG snapshot을 owner에게만 제공하며 input 부재는404, 인증 누락은401,
쿼리/다른 origin은403이다. M4g-29는 APNG의 IDAT 정적 기본 이미지도 받는다.
원본80MiB/8192px축/16Mpx/65536chunks 제한, 모든 chunk CRC·길이·trailing data 검증
후 메모리에서만 `acTL/fcTL/fdAT`를 제거한다. 정적 PNG와 그 외 chunk/metadata는
바이트 불변이다. `bytes`는 제공 snapshot 길이이며 API 스키마는 같다. 애니메이션의
제어/순서 의미 검증·재생은 하지 않는다. 실제 픽셀 디코딩은 브라우저에 맡긴다.
