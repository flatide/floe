...
- **남은 구현 비용(블록 경계마다 디코드 배치)**: `load_cancellable`이 호출마다 디코드 워커를 세운다.
  같은 페이지를 읽는 ordered:1이 baseline보다 디코드가 2.2배 느리다(449 fit 149 → 323 ms). 블록을
  키우면 줄지만(N=8에서 215 ms) 남는다. 이걸 없애면(디코드 풀 유지) occlusion의 이득이 더 커진다 —
  다음 단계 후보 1순위.
