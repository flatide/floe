...
원인은 박스가 아니다: ① 래스터의 픽셀 정책(KLayout hairline parity — 픽셀보다 작은 도형은 축마다
가장 가까운 픽셀 하나로 접혀 **반드시 1픽셀을 켠다**)이라 703 µm부터 멤버 520개가 정확히 520픽셀을
켜고, pitch가 1 px 아래로 내려가면 전부 이어져 꽉 찬 사각형이 된다. ② 플래너는 repetition 레코드를
**배열 전체의 크기**로 size cut하므로 배열 전체가 컷보다 작아질 때(11,787 µm)까지 모든 멤버가 그리기
대상이다. 사용자의 판단("대상이 탈락하지 않는다, 픽셀 마스크가 두 번 그리지 않을 뿐")이 맞다.
