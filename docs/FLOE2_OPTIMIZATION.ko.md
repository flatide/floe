...
순서만 바꾼 값(합성 칩 1/10, keep medium, **prepare+paint** ms, cold): 1·16 레이어는 사실상 같고
(16 레이어 fit 179 → 184), 64·449 레이어는 느려진다(449 fit 180 → 273, ×4 286 → 383). 장벽이 타일 간
부하 분산을 없애기 때문이다(워커 1개면 두 모드가 같다). 0.12.176의 **블록 실행**(연속 N개 pass를 한
타일에 몰아 그리고 블록 끝에서만 동기화)이 그 초과분을 대략 절반으로 줄인다(449 fit block 8에서 228,
×4 328). 리뷰 2026-09-20의 계측 보정 2건을 반영한 수치다: ① baseline은 준비를 자기 렌더 호출 안에서
하므로 paint끼리 비교하면 안 된다(이 보정으로 "16 레이어 3배" 주장은 철회했다), ② (뷰, 모드)마다 새
워커여야 cold다. 3단계가 아낄 디코드 106~260 ms는 **상한**이다 — 픽셀을 못 쓴 페이지라도 bbox가 열린
픽셀에 걸치면 사전 생략할 수 없다.

#### F2R-28 다음 단계 2·3 — 메타 장면과 실제 디코드 생략 (0.12.177, 2026-09-20)

`render_probe mode=occlusion`: 블록 시작 마스크로 그 블록의 레이어가 열린 픽셀에 닿을 수 있는
페이지만 읽는다. 준비물은 **플랜의 모든 페이지를 아는 장면**이다(`FrameScene::new_metadata`,
`Cache::page_geometry`) — 메인 bin·미니 bin·하위 레이어 마스크가 디코드 전에 만들어지고, 페이지는
`set_decoded_page`로 나중에 슬롯에 들어간다. 판정은 `BlockDemand::pages_for_plane`이고, 어느
인스턴스라도 열릴 가능성이 있으면 읽으며 deferred edge는 그 레이어를 통째로 읽는다.

세 모드의 프레임은 같다(게이트 `layer_decode`: 112개 뷰/모드/블록 조합 바이트 동일). 합성 칩 1/10,
1920×1080, keep, medium, cold 중앙값(3회, 모드 순서 회전):

| 레이어 | 뷰 | baseline | ordered:8 | occlusion:8 | 읽은 페이지 | 안 읽은 stored |
|---|---|---|---|---|---|---|
| 16 | fit / ×4 / ×8 | 495 / 1110 / 664 | 507 / 1137 / 699 | 507 / 1148 / 696 | 25~36 / 31~155 | 0~0.4 MB |
| 64 | fit / ×4 / ×8 | 134 / 191 / 96 | 146 / 270 / 148 | 142 / 258 / 130 | 118 / 78 / 52 (233·323·823 중) | 1.5~3.1 MB |
| 449 | fit / ×4 / ×8 | 470 / 669 / 331 | 585 / 808 / 436 | **535 / 583 / 119** | 403 / 159 / 69 (1579·2218·5324 중) | 17~62 MB |

- **가림은 실재한다**: 449 레이어에서 읽는 페이지가 1/4~1/77로 준다. 디코드 메모리도 903 → 212 MB
  (×4)로 줄어 1024 MB 예산에 직접 걸린다. 가림 조회 자체는 프레임당 0.1~0.8 ms다.
- **같은 N끼리**(순서 비용을 뺀 순이익) 449 레이어: fit 757→657(N=1) / 636→571(4) / 585→535(8),
  ×4 1126→686 / 919→621 / 808→583, ×8 684→163 / 527→140 / 436→119.
- **baseline 대비 최종**: 449 레이어 ×4 −13 %, ×8 −64 %. fit은 +14 %로 여전히 느리고, 64·16
  레이어는 전부 느리다. 즉 지금 형태로 이기는 구간은 **레이어가 많고 가림이 많은 근접 뷰**다.
- **블록 경계마다 디코드 배치(0.12.178에서 해소)**: `load_cancellable`이 호출마다 디코드 워커를
  세웠다. 페이지당 일(`page_decode_sum_us`)은 모드마다 같은데 wall만 2배였다 — 잃은 것은 **병렬성**
  이다(배치가 작으면 워커를 다 쓰지 못하고, 배치마다 세우고 접는다). `Cache::with_decode_pool`이
  프레임 내내 워커를 유지하고 `DecodedPageCache::load_pooled`가 그 위에서 배치를 돌린다(워커 패닉은
  그 배치의 오류가 되고, 본체가 패닉해도 워커는 Drop에서 풀린다). 449 레이어에서 ordered:8의 디코드
  207 → 153(fit) / 335 → 267(×4) / 339 → 279(×8) ms, occlusion:8은 162 → 126 / 117 → 100 / 38 → 28.
  같은 실행 안에서 baseline과의 차이는 +48 / +20 / −35 ms로 줄었다(전에는 +58 / +87 / +90).
- 블록 크기는 **장벽이 가장 적은 크기가 아니라 전체가 가장 짧은 크기**로 고른다: 여기서는 N=8이
  둘 다 최선이었다(더 읽는 페이지 403 대 388은 장벽 3개보다 싸다). 적응형은 아직 넣지 않았다.

#### F2R-28 계측 보정 2건과 고정 블록 비교 (0.12.179, 2026-09-21)

리뷰 지적 2건을 고쳤다.
1. **디코드 풀 생성·종료가 paint에 들어갔다.** `with_decode_pool`이 그 시간을 따로 돌려주고
   (`pool_us`), 드라이버가 decode·demand와 함께 paint에서 뺀다. baseline은 워커 생성·종료를
   decode 안에서 하므로 분류가 달랐다.
2. **캐시가 전부 hit인 블록도 워커를 깨웠다.** `DecodePool::decode_pages`가 취소 확인은 유지한 채
   빈 배치에서 바로 돌아온다. warm 실행에서 블록마다 배리어 두 번을 치르던 비용이다.

고정 블록 비교(449 레이어, cold 중앙값 4회). **주의: 측정 중 원격 세션이 CPU 176 %를 쓰고 있어
(load 8~10) 실행 간 편차가 크다(범위를 함께 적는다). 시간은 같은 실행 안에서만 비교한다.**

| 뷰 | baseline | ordered:1000 | ordered:32 | ordered:8 | occl:1000 | occl:32 | occl:8 |
|---|---|---|---|---|---|---|---|
| fit 총 ms | 466 | 531 | 509 | 522 | 502 | 497 | 591 |
| fit 범위 | 438~470 | 399~901 | 454~523 | 481~639 | 431~550 | 479~529 | 437~709 |
| fit 읽은 페이지 | 1579 | 1579 | 1579 | 1579 | 1579 | 456 | 403 |
| ×4 총 ms | 777 | 691 | 687 | 830 | 705 | **510** | 589 |
| ×4 범위 | 620~1037 | 666~752 | 605~730 | 682~871 | 660~714 | 498~529 | 565~666 |
| ×4 읽은 페이지 | 2218 | 2218 | 2218 | 2218 | 2218 | 194 | 159 |

부하와 무관한 신호만 읽으면:
- **블록 하나(`:1000`)는 가림을 전혀 쓰지 못한다**(1579·2218 전부 읽는다). 아무것도 그려지지 않은
  시점에 한 번 판정하기 때문이다 — 메타 장면·세션의 비용만 남는 대조군이다.
- 블록이 커지면 더 읽는다: ×4에서 8 → 32 → 1000이 159 → 194 → 2218 페이지.
- ×4에서는 그 차이를 감수하고 **32가 8보다 빠르다**(510 대 589): 장벽 4배가 페이지 35개보다 비싸다.
  fit은 전부 편차 안이라 판단하지 않는다.
- 블록 크기 규칙은 실칩 결과 뒤에 정한다(합성 파일에 맞춘 정책이 될 위험).

#### F2R-28 실칩 결과 (2026-09-21, 사용자 측정, `MEASURED`) — 가림으로 생략되는 페이지가 거의 없다

사용자가 폐쇄망의 실칩에서 `bench_layer_decode.py --modes baseline,ordered:8,occlusion:8 --layers all
--zooms 1,4,8 --repeat 3 --warm 2`를 돌려 `== type this ==` 블록을 옮겨 적었다(칩·뷰 중심은 기록
전). 두 depth 모두 `uns=0`(deferred edge 때문에 레이어를 통째로 읽은 경우 없음).

| depth | 줌 | sel | occlusion이 읽은 페이지 | 생략 | bas | ord | occ | warm bas / occ |
|---|---|---|---|---|---|---|---|---|
| full | fit | 1898 | 1892 | 0.3 % | 156 | 185 | 242 | 148 / 188 |
| full | ×4 | 608 | 608 | 0 % | 115 | 136 | 147 | 89 / 100 |
| full | ×8 | 523 | 521 | 0.4 % | 145 | 163 | 193 | 129 / 154 |
| 0 | fit / ×4 / ×8 | 3 / 3 / 5 | 전부 | 0 % | 70 / 73 / 80 | 70 / 75 / 84 | 71 / 76 / 85 | 59~73 / 64~75 |

판정:
- **실칩에서는 위 레이어가 페이지를 덮는 일이 거의 없다.** 합성 칩은 같은 모드로 1/4~1/77만 읽었는데
  (−17 % ×4, −73 % ×8) 실칩은 0~0.4 %다. `uns=0`이므로 deferred 경로 탓이 아니라, 모든 페이지의
  bbox가 판정 시점에 아직 열린 픽셀에 닿는다는 뜻이다. 합성 생성기(`gen_main01_like.py`)의 block
  역할 레이어가 셀 전체를 덮는 도형을 만들어 가림을 과장했다 — 합성 결과를 실칩 성과로 옮기면 안 된다.
- 그래서 occlusion은 **비용만 남는다**: baseline 대비 +28~55 %(cold), +12~27 %(warm). 같은 블록의
  ordered와 비교해도 +11~57 ms — 아무것도 못 찾는 가림 조회(인스턴스마다 페이지 bbox 검사)의 값이다.
  순서 자체(`ord − bas`)는 +12~19 %.
- depth 0은 선택 페이지가 3~5개라 어느 모드든 차이가 없다.
- 이 뷰들의 baseline 자체가 115~156 ms로 디코드가 병목이 아니다. 디코드가 병목인 뷰(예산 1024 MB에
  걸리는 밀집 뷰)는 아직 재지 않았다.
- **결론: `render_probe`는 진단 경로로 남기고 기본 경로에 넣지 않는다.** 블록 크기 규칙·자동 선택도
  만들지 않는다(실칩 근거가 없다). 다시 볼 조건: 디코드가 병목인 실칩 뷰에서 `pg`가 `sel`보다 크게
  작게 나올 때. 원인을 가르고 싶으면(덮는 레이어가 없어서인지, bbox가 거칠어서인지, 기본 speckle
  채움이 덮지 못해서인지) 판정 시점의 채워진 픽셀 비율과 조회 후보 수를 digest에 더하면 된다.

#### 알려진 문제 → 박스 기본 끔: 보이는 레이어가 적을 때 sub-cut 박스 계획이 수십 초 (2026-09-21, 실칩 확인, 0.12.182)

사용자 관찰(MAIN01, 449 레이어, detail medium, thin keep, full depth): 전 레이어를 켜면 어느 뷰든
1초 미만인데, 가장 위에 그려지는 10개만 켜면 fit에 가까울수록 느려져 20초를 넘는다.
`FLOE_RUST_SUB_CUT_BOX=off`로 빨라지는 것을 사용자가 확인했다.

원인(합성 칩 1/10에서 깊은 셀의 레이어 10개로 재현 — fit 5.42 s 대 0.05 s, 계획 4970 ms):
① 박스는 보이는 레이어가 16개 이하일 때만 켜진다(449개면 꺼진다). ② 켜지면 플래너가 컷에 걸린
서브트리를 노드째 자르지 않고 배치까지 걸어 박스를 만든다(방문 노드 11만 → 94만, 레이어 확인 읽기
460만, 박스 181만). ③ 박스가 200만을 넘으면 한 단계 거친 박스로 **프레임 전체를 다시 계획한다**
(최대 3번). 그 레이어들은 fit에서 선택 페이지가 0이라 화면의 전부가 박스다 — 끄면 빈 화면이 된다.

결정(사용자): **thin keep에서 박스를 끄고, 컷 아래는 밀도 표현으로 해결한다**(F2R-30, 인덱스에 미리
계산한 덮임 — 그러면 이 걷기 자체가 없어진다). 0.12.182부터 박스는 기본으로 꺼져 있고
`FLOE_RUST_SUB_CUT_BOX=on`일 때만 켜진다(진단용; 플래너 코드와 게이트는 남긴다). 밀도 표현이 들어오기
전까지는 보이는 레이어가 적은 넓은 뷰에서 컷 아래 도형이 대체 표시 없이 비어 보인다(0.12.168 이전과
같다). 밀도 표현의 비용은 화면 픽셀 수에 비례해야 하며, 이 사례(적은 레이어 × 깊은 셀 × fit)를 그
검증 뷰로 쓴다.
