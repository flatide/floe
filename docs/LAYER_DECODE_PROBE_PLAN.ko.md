...
## 11. 1단계 결과 (0.12.175, 2026-09-20)

픽셀 계약은 통과했다. 단위 테스트(`write_once_frames_match_the_ordered_overwrite_byte_for_byte`에
붙였다)에서 36개 장면 × bin/walk × write-once on/off의 세션 프레임이 기존 경로와 바이트 동일하고,
write-once가 건너뛴 타일·pass·항목 수도 같다. 게이트는 §9의 1~5·8과 published scene·occlusion 거절을
덮는다(6·7·9는 2·3단계에서).

**그리는 순서만 바꾼 비용**(합성 칩 1/10, 1920×1080, keep, medium, frames·labels 끔, 래스터 워커 4,
`baseline → ordered`의 paint ms, cold):

| 레이어 | fit | ×4 | ×8 |
|---|---|---|---|
| 1 | 44 → 33 | 71 → 35 | 36 → 25 |
| 16 | 176 → **61** | 100 → 99 | 76 → 49 |
| 64 | 69 → 91 | 124 → 190 | 28 → 51 |
| 449 | 190 → 292 | 302 → 406 | 27 → 53 |

순서 변경은 **일률적으로 비싸지 않다**. 레이어가 적고 항목이 많은 뷰(16 레이어 fit: sub-cut 박스 수십만
개가 몇 개의 plane에 몰린다)에서는 3배 빨라진다 — 모든 타일이 같은 plane을 연달아 그려 항목 목록과
페이지가 캐시에 남기 때문으로 보인다(미검증). 레이어가 많으면 느려진다: pass마다 워커를 깨워 기다리는
동기화(449 pass)와, pass 단위 장벽이 **타일 간 부하 분산을 없애는 것**(프레임 시간이 max(타일별 합)에서
Σ max(타일)로 바뀐다)이다. 스레드 생성은 이미 없앴고(워커 풀을 프레임 내내 유지) 남은 비용이므로
구현 문제가 아니다 — 워커를 1개로 두면 두 모드의 paint가 같다(542 대 517 ms).
449 레이어에서 +100 ms 안팎이고, 3단계가 아낄 수 있는 디코드는 106~260 ms(F2R-28: 그중 92~99 %가
한 픽셀도 못 쓴다)이므로 **장벽이 상금의 절반쯤을 먹는다**. 줄이려면 레이어를 묶어 한 번에 판정하는
방법(블록 단위 가림 판정, 보수적이지만 장벽이 1/N)이 있고 3단계 전에 정해야 한다.

디코드는 두 모드가 같다(1단계는 아무것도 생략하지 않는다).
