...
- 실칩에서 돌릴 것(대표 뷰마다, depth full과 0 둘 다):
  ...
  **그 블록의 줄만 옮기면 되고 나머지 출력은 옮길 필요가 없다.** 값이 없는 열은 머리글로
  접힌다(`uns=0`), `--warm 0`이면 warm 열이 빠진다. 한 depth당 4줄이다:
      ...
      == type this ==
      # d=f n=8 uns=0
      z sel bas ord occ pg
      1 1579 355 456 414 403
      4 2218 622 752 466 159

  `uns`(deferred edge라 그 레이어를 통째로 읽은 수)와 `pg` 대 `sel`이 판단의 핵심이다 — 실제
  공유 인스턴스와 deferred 배열에서 생략이 얼마나 남는지가 거기 드러난다. `ord`는 같은 블록
  크기로 **아무것도 생략하지 않은** 대조군이라 `ord − bas`가 순서 비용, `occ − ord`가 생략의
  순이익이다.
