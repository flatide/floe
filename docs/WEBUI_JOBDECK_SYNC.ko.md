...
## 후속 구현: 명시 Index에서만 개명

GTK 최신 구현은 열기에서도 구 이름을 자동 개명한다. 웹의 읽기 범위는
그것만으로 파일시스템 변경 권한을 얻지 않는다. 현재 구현은 읽기/재사용 시
**개명하지 않으며**, 명시 index도 선택된 기존 경로를 재사용한다.
2026-09-17 사용자는 **명시 Index에서만 개명**을 승인했다. 현재 통합의 회귀를
...
이번 동기화와 회귀는 닫았고, 승인된 명시 Index 개명 구현이 다음 로컬 단계다.
