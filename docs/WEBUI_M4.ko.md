...
## 102. M4g-45 — 실제 Chrome 내장 두벌식 입력 수용

2026-09-18. [브라우저 §9.1](WEBUI_BROWSER_ACCEPTANCE.ko.md#91-내장-두벌식--실제-키커서초안-폐기)에
보존된 합성62804의 실제 key 입력 결과를 기록했다. 조합·받침/중성 삭제·중간 삽입·
선택 교체·UTF-16 emoji 커서·키보드/체크박스 모드 전환·뷰어 단축키 보호·폐기 후
빈 초안/모드off 재열기를 확인했다. screenshot의 한글 표시와 saved-note 읽기 복구도
확인했다. 승인/저장 없이 원상 복원했고10개 입력/기존 sidecar의 hash는 불변이다.

문서만 변경했다. 제품 회귀는 §101의 직전 결과이며 새 전체 실행으로 계산하지 않는다.
...
별도 [브라우저 §5.3](WEBUI_BROWSER_ACCEPTANCE.ko.md#53-도형-선택스냅-좌표-대조와-룰러-버튼-이동)은
원본과 사각형1개의 면적/좌표가 일치한 근거다. 제어 연결 상실로 테스트 뷰 복원이
당시 남았으며 서버를 재시작하거나 다른 방식으로 인증/파일 권한을 우회하지 않았다.
후속 연결 복구와 복원/스냅 검사는 §104에 기록했다.

전체 목표 잔여: 실제 브라우저 나머지 입력·설정·공유 및 다중 저장·충돌/불명확한
게시 복구, Python-free Linux 실행, G1/G4 최종 대조, 현장 Firefox/ETX다.
원격 SH-10/index hot reload는 사용자 보류, M5는 실측 조건부다.

## 104. M4g-47 — 실제 스냅 측정과 움직이는 룰러 버튼

2026-09-18. 보존된 합성62804에서 꼭짓점/변 스냅,18µm 사각형 폭과2µm 배열 간격을
원본 좌표와 대조했다. 자동 bbox gap/Shift 자유각 전체 수용은 아니다. 검사가 끝난 뒤
뷰/레이어/오버레이를 복원했고 기존10파일은 불변이다. DRC 저장은 없다.

probe 결과가 사라질 때 룰러 버튼이55.59 CSS px 이동해 클릭이 빗나갔다. 스냅 표시
영역을2줄 고정 높이로 두어 내용 유무에 따른 reflow를 막고, 긴 결과는 키보드 초점이
가능한 내부 스크롤 영역에 보존한다. 질의·스냅·측정 계산 자체는 변경하지 않았다.
source guard 및 `sh tools/validate_rust.sh --only web_ui`, offline/locked release
app 빌드가 통과했다. 기존 native 경고는 그대로이며 native 전체 배터리 재실행은 아니다.

에이전트가 수정본 서버51199를 직접 실행했지만 브라우저의 비공개 시작 파일 접근이
URL 정책으로 차단됐다. 공개 비인증 화면의39.20px 예약 높이만 확인했으며 인증/보안
설정을 우회하지 않았다. 수정 후 실제 스냅→버튼 클릭 재검증은 별도다
([브라우저 §5.3](WEBUI_BROWSER_ACCEPTANCE.ko.md#53-도형-선택스냅-좌표-대조와-룰러-버튼-이동)).
...
## 112. M4g-55 — 실제 Chrome 겹침 후보·45도 PATH 선택

2026-09-18. 기존 승인 합성62804 세션에서 세 사각형의1→2→3→1 및 역방향 순환,
레이어 숨김 후3→2 후보 감소와 빈 영역 no-hit를 확인했다. 추가로45도 PATH의
bbox/면적이 KLayout 원본과 일치하고 선택 윤곽이 실제 대각선 형상을 따르며,
bbox 안이지만 도형 밖인 클릭은 선택되지 않음을 확인했다. 상세 수치·입력·범위는
[브라우저 §5.6](WEBUI_BROWSER_ACCEPTANCE.ko.md#56-실제-겹침-후보-순환가시성45도-path)에 기록했다.

제품 코드 변경은 없다. 현재 HEAD의 `node rust/web/ui/inspect.test.cjs`와
`node rust/web/ui/query.test.cjs` 모두 ALL OK. 문서만 바뀌므로 native 전체
배터리는 재실행하지 않았다. 카메라/9레이어/DRC/룰러 상태를 복원했고 기존 입력·
캐시·리뷰10파일 해시가 불변이다. 저장/다운로드/새 인증이나 서버 재시작을 하지 않았다.

전체 목표 잔여: 최신 빌드의 CSS/초기화/BFCache 및 나머지 실제 입력·설정·공유/
저장·충돌/결과 불명 수용, Python-free Linux 실행, G1 성능·G4 최종 판정,
현장 Firefox/ETX다. 원격 SH-10/index hot reload 보류·M5 실측 조건부와 별도 합성
다중 저장/CI 실행 승인 대기는 유지한다. 이전 실행 파일의 일부 형상 선택 수용을
최신 빌드 전체 또는 전체 goal 완료로 확대하지 않는다.

## 113. M4g-56 — 늦은 뷰 닫기 응답과 닫힌 화면 복원

2026-09-18. 실제 `app.js`를 사용하는 결정적 하네스에서 두 경로를 재현했다.
이전 뷰의 DELETE 응답이 늦으면 BFCache/연결 복구로 열린 새 뷰의 소켓을 닫고
프레임을 지웠다(`readyState 1 → 3`). 별도로 복귀 조회가 `closed`를 반환해도
이전 프레임을 숨기지 않았다. 수정 전 두 assertion이 실패했으며 테스트의 HTTP
성공 상태는 native `owner::close_view` 계약인202로 맞췄다.

- Close 결과의 UI 적용을 요청 당시 `pageRun`과 `view_id`에 한정한다.
  오래된 성공/실패가 새 화면이나 종료/종료 불명 문구를 덮지 않는다. 요청을
  취소했다고 주장하거나 자동으로 다시 제출하지 않는다.
- 정상 Close 수락, 현재 소켓의 `closed` snapshot, 복귀 GET의 `closed`/404가
  같은 표시 정리 함수를 쓴다. foreground/margin·도형 선택·스타일 편집기와
  이전 Live/perf 표시를 비우고 소스 재열기를 허용한다. 이미 교체된 뷰의
  늦은 Close는 이 함수를 호출하지 않는다. 새 세션 생성/파일 쓰기는 없다.
- 현재 페이지의401은 여전히 세션 만료로 처리한다. 페이지 세대가 지난401과
  같은 페이지에서 뷰만 바뀐401을 구분하며 인증 오류를 모두 삼키지 않는다.

검증: `validate_web_ui.cjs`에 현재/숨김/BFCache/동일 페이지 재연결/정상 종료/
종료 불명 ×202/503/401의18조합, absent/closed/소켓 알림3조합을 배선했다.
전체 UI 및 ES2017 게이트 통과. release 재빌드 뒤 `validate_web_cli.py`를
새 합성 valmini로 직접 실행해 내장 `app.js` 바이트 일치,202 닫기→closed 조회→
다른 `view_id` 재열기, 기존 기동/인덱싱/종료·자식 수거를 확인했다.
정규 진입점 `sh tools/validate_rust.sh --only web_cli,web_ui`도 exit0/ALL OK다.
이번 실행용 `.venv` 연결은 제거했고 합성 입력만 정리했다.
기존 브라우저 서버와 main/jobdeck 작업은 변경하지 않았다. 전체 Rust 배터리나
실제 Chrome의 지연 응답/BFCache 검증으로 확대하지 않는다.

전체 목표 잔여: 최신 브라우저의 입력·설정·공유/BFCache 및 저장/충돌/결과 불명
수용, Python-free Linux 실행, G1 성능·G4 최종 판정, 현장 Firefox/ETX.
원격 SH-10/index hot reload 보류, M5 실측 조건부, 합성 다중 저장/CI 승인 대기는
유지한다. 이번 두 경로의 수정으로 복원 전체나 웹 전환 전체 완료를 주장하지 않는다.

## 114. M4g-57 — 동일 페이지의 복원 조회 응답 역전

2026-09-18. M4g-56의 Close 응답과 달리 이번 대상은 `GET /api/v1/view`다.
같은 페이지에서 연결 복구와 launcher 완료의 읽기 경로가 겹치면 `pageRun`만으로
오래된 조회를 구분하지 못했다. 새 뷰 연결 뒤 옛 응답을 반환하는 결정적 검사에서
소켓이2개→3개로 늘고 새 표시가 퇴역했다. Close 수락 뒤, 새 연결 뒤, 같은 소켓의
최신 snapshot 뒤에 반환하는 세 경우도 수정 전 assertion이 각각 실패했다.

- 복원 조회마다 순번과 시작 당시 소켓 세대를 포착한다. 새 조회·새 연결·명시적인
  뷰 정리가 선행했으면 이전 조회의 성공/실패는 UI를 바꾸지 않는다. 서버 작업이나
  닫기를 다시 제출하지 않으며 기존 프레임 일시 보관 정책도 바꾸지 않는다.
- 같은 `view_id`는 응답 `state_rev`가 현재 snapshot보다 낮으면 무시한다.
  렌더 실패/진행은 revision이 같을 수 있으므로 조회 이후 snapshot을 받았다면
  같은 revision의 응답도 그 상태를 덮지 않는다. 이 실패 상태 역전도 별도 재현했다.
  revision 없는404는 조회 시작 뒤 상태가 갱신됐다면 그 최신 관측을 지우지 않는다.
  새 조회의404/closed 또는 현재 소켓의closed 알림은 기존 닫기 정리를 계속 수행한다.
- 현재 조회의503 오류는 표시하고, 현재 페이지의401은 원래 HTTP 인증 처리대로
  세션 만료다. 더 높은 revision을 가진 정상 조회는 수용한다. 과거 결과를 버리기
  위해 모든 오류를 삼키거나 모든 복원 요청을 막는 변경이 아니다.
- 버려진 조회는 호출자에게도 미적용을 반환한다. mode 완료 처리에서 오래된
  성공 응답을 근거로 최신 렌더 실패 문구를 지우지 않는다. 상태 역전을 막은 뒤에도
  이 후처리가 문구를 지우는 `phase/old` 실패를 발견해 호출 경로까지 수정했다.

검증: 실제 `app.js` 하네스에서 새 복원/Close/새 소켓/새 revision snapshot/
동일 revision 실패 snapshot/현재 요청과 옛 정상 응답/closed/404/503/401을
대조하는30조합, 최신 정상 revision 수용3조합을 `validate_web_ui.cjs`에 배선했다.
최종 코드에서 집중33조합과 `sh tools/validate_rust.sh --only web_cli,web_ui`가
모두 통과했다(exit0). 앞선 실행의 `phase/old` 문구 보존 실패를 제외하거나 기대값을
완화하지 않았다. native 게이트는 서버를 직접 실행해 내장 `app.js` 바이트 일치,
첫 프레임, `202 → closed → 새 view_id`, 로그아웃/SIGINT와 자식·임시 파일 정리를
대조한다. 정규 UI 게이트도 전부 통과했다. 이 결과는 선택한 두 게이트의 근거이며
미실행 전체 Rust 배터리나 실제 Chrome 응답 역전/물리 표시·BFCache 수용은 아니다.

전체 목표 잔여는 최신 브라우저 입력·설정·공유/BFCache 및 저장/충돌/결과 불명
